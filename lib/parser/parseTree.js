import Rogain from '../core/rogain.js';

export default function parseTree(dom, opts) {
  var inst = opts && opts.rogainInstance ? opts.rogainInstance : new Rogain();
  var tree = { type: getTreeType(dom, inst) };
  ...
  if (dom.children) {
    tree.children = dom.children.map(child => parseTree(child, opts));
  }
...
function getTreeType(dom, inst) {
  if (dom.type === 'tag') {
    ...
    if (first === first.toUpperCase()) {
      if (inst.helpers[dom.name] !== undefined) {
        return 'helper';
      } else {
        return 'component';
      }
